...
class ConsolaCentral(stomp.ConnectionListener):
    ...
    def printOffice1IlluminationInformation(self, data):
        ...
        if (illumination < self.INCREASE_INTENSITY_ILLUMINATION_REGULATOR_ACTIVATION_INTENSITY and not self.getIncreaseIntensityRegulatorActivatedFlagFromMessage(data)):
            print("Illumination in Office1 is below threshold. Requesting Office1 to activate Increase Intensity Regulator...")
        if (illumination >= self.INCREASE_INTENSITY_ILLUMINATION_REGULATOR_STOP_INTENSITY and self.getIncreaseIntensityRegulatorActivatedFlagFromMessage(data)):
            print("Increase Intensity Regulator is activated in Office1.")
        if (illumination > self.DECREASE_INTENSITY_ILLUMINATION_REGULATOR_ACTIVATION_INTENSITY and not self.getDecreaseIntensityRegulatorActivatedFlagFromMessage(data)):
            print("Illumination in Office1 is above threshold. Requesting Office1 to activate Decrease Intensity Regulator...")
        if (illumination <= self.DECREASE_INTENSITY_ILLUMINATION_REGULATOR_STOP_INTENSITY and self.getDecreaseIntensityRegulatorActivatedFlagFromMessage(data)):
